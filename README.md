...
`Casks/<token>.rb`, commits and pushes. Pushing to origin is what publishes:
this repository is a push mirror to GitHub, GitHub is never pushed
...
- Claude Counter: `bin/release-claude-counter.sh <version>` tags the checkout,
  pushes the tag to origin (the mirror carries it to GitHub), builds with
  Developer ID and calls `publish-app.sh`. The tag must reach GitHub through
  origin: the mirror is `--mirror --force` and would delete a tag made on GitHub.
